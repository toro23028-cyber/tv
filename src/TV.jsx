...
// HELPERS
function getSchedule(chId){const p=PROGRAMS[chId]||[];if(!p.length)return[];const s=[];let c=0,i=0;while(c<86400){const x=p[i%p.length];const sH=Math.floor(c/3600),sM=Math.floor((c%3600)/60);const e=c+x.duracao;const eH=Math.floor(e/3600),eM=Math.floor((e%3600)/60);s.push({...x,id:`${x.id}_${i}`,horarioInicio:c,horarioFim:e,horarioTexto:`${String(sH).padStart(2,"0")}:${String(sM).padStart(2,"0")}`,horarioFimTexto:`${String(eH%24).padStart(2,"0")}:${String(eM).padStart(2,"0")}`});c=e;i++}return s}
function getNow(){const n=new Date();return n.getHours()*3600+n.getMinutes()*60+n.getSeconds()}
function getCurProg(chId){const s=getNow();return getSchedule(chId).find(p=>s>=p.horarioInicio&&s<p.horarioFim)}
function getElapsed(p){return getNow()-p.horarioInicio}
...
// SMALL COMPONENTS
function LiveDot(){const[v,setV]=useState(true);useEffect(()=>{const i=setInterval(()=>setV(x=>!x),800);return()=>clearInterval(i)},[]);return <span style={{display:"inline-flex",alignItems:"center",gap:4,fontSize:11,fontWeight:700,color:"#ff3b3b",opacity:v?1:0.3,transition:"opacity 0.3s"}2}><span style={{width:8,height:8,borderRadius:"50%",background:"#ff3b3b",boxShadow:"0 0 6px #ff3b3b"}}/>AO VIVO</span>}
...
function shareProgram(prog,ch){const text=`📺 ${prog.nome}\n🕐 ${prog.horarioTexto} - ${prog.horarioFimTexto}\n📡 ${ch?.nome||"TVWEB"}\n\n${prog.sinopse||""}`;if(navigator.share)navigator.share({title:prog.nome,text,url:window.location.href}).catch(()=>{});else{navigator.clipboard?.writeText(text);alert("Copiado!")}}

function scheduleNotif(prog,ch,min=5){const ns=getNow();const ts=prog.horarioInicio-min*60;const delay=(ts-ns)*1000;if(delay<=0){alert("Programa já começou!");return}if(!("Notification"in window)){alert("Navegador sem suporte.");return}Notification.requestPermission().then(p=>{if(p!=="granted")return;setTimeout(()=>{new Notification(`📺 ${prog.nome} em ${min}min!`,{body:`${ch?.nome} · ${prog.horarioTexto}`})},delay);alert(`✅ Lembrete: ${min}min antes de "${prog.nome}"`)})}

function FsBtn({cRef}){const[fs,setFs]=useState(false);const[pulse,setPulse]=pulse=useState(true);useEffect(()=>{const h=()=>setFs(!!document.fullscreenElement);document.addEventListener("fullscreenchange",h);const t=setTimeout(()=>setPulse(false),6000);return()=>{document.removeEventListener("fullscreenchange",h);clearTimeout(t)}},[]);return <button onClick={()=>{if(!document.fullscreenElement)cRef.current?.requestFullscreen?.();else document.exitFullscreen?.()}} style={{position:"absolute",top:16,left:"50%",transform:"translateX(-50%)",zIndex:15,background:"rgba(0,0,0,0.5)",border:"1px solid rgba(255,255,255,0.15)",color:"#ccc",padding:"6px 14px",borderRadius:20,cursor:"pointer",fontSize:11,fontWeight:600,animation:pulse?"pulseFull 2s ease infinite":"none"}}>{fs?"↙ Sair":"↗ Tela Cheia"}</button>}
...
function EPGCompact({channels,currentChannelId,onSelectChannel,onSelectProgram,onOpenFull,onClose}){
  ...
  const timeMarks=[];
  for(let i=0;i<96;i++){const h=Math.floor(i/4),m=(i%4)*15;timeMarks.push({label:m===0?`${String(h).padStart(2,"0")}:00`:`${String(h).padStart(2,"0")}:${String(m).padStart(2,"0")}`,isFull:m===0})}
            ...
            {sched.filter(p=>p.horarioFim<=86400).map(prog=>{
              ...
              return <div key={prog.id} onClick={()=>{onSelectChannel(ch.id);onSelectProgram(prog)}}
                style={{
                  ...
                  display:"flex",flexDirection:"column",justifyContent:"center",transition:"background 0.2s",
                }}
    ...
    <div onClick={e=>e.stopPropagation()} style={{maxWidth:720,margin:"0 auto",padding:20,minHeight:"100vh"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:6}}>
        {sched.filter(p=>p.horarioFim<=86400).map(prog=>{
          ...
          return <div key={prog.id} onClick={()=>onProgramClick(prog)} style={{
...
export default function TVWeb(){
  ...
  const swCh=useCallback(id=>{if(id===curCh)return;setFade(true);setTimeout(()=>{setCurCh(id);setFade(false)},300);setInfo(true);rHide()},[curCh]);
  const swDir=useCallback(dir=>{const i=CHANNELS.findIndex(c=>c.id===curCh);let n;if(dir>0)n=i<CHANNELS.length-1?CHANNELS[i+1].id:CHANNELS[0].id;else n=i>0?CHANNELS[i-1].id:CHANNELS[CHANNELS.length-1].id;swCh(n)},[curCh,swCh]);
  const rHide=useCallback(()=>{clearTimeout(hRef.current);setInfo(true);hRef.current=setTimeout(()=>{if(!showEPG&&!showFull)setInfo(false)},5000)},[showEPG,showFull]);
