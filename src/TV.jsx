  ...
  return <div onClick={e=>e.stopPropagation()} style={{position:"absolute",bottom:0,left:0,right:0,zIndex:20,animation:"slideUp 0.3s ease"}}>
    ...
    <div style={{background:"rgba(10,12,18,0.98)",display:"flex",overflow:"hidden",maxHeight:"60vh",minHeight:320}}>
      <div style={{minWidth:140,borderRight:"1px solid rgba(255,255,255,0.08)",flexShrink:0,overflowY:"auto"}}>
        ...
        {channels.filter(ch=>ch.id===currentChannelId).map(ch=><div key={ch.id} onClick={()=>onSelectChannel(ch.id)} style={{height:ROW_H,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",borderBottom:"1px solid rgba(255,255,255,0.05)",background:"rgba(26,115,232,0.15)",borderLeft:"3px solid #1a73e8"}}>
          <div style={{textAlign:"center"}}><ChLogo ch={ch} size={36}/><div style={{fontSize:12,fontWeight:600,color:"#fff",marginTop:4}}>{ch.nome}</div></div>
        </div>)}
        {/* Other channels below and above */}
        {channels.filter(ch=>ch.id!==currentChannelId).map(ch=><div key={ch.id} onClick={()=>onSelectChannel(ch.id)} style={{height:ROW_H,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",borderBottom:"1px solid rgba(255,255,255,0.05)",background:"transparent"}}>
          <div style={{textAlign:"center"}}><ChLogo ch={ch} size={36}/><div style={{fontSize:12,fontWeight:600,color:"#888",marginTop:4}}>{ch.nome}</div></div>
        </div>)}
      </div>
      <div ref={scrollRef} style={{flex:1,overflowX:"auto",overflowY:"hidden",position:"relative"}}>
        ...
        {/* Current channel first in schedule */}
        {channels.filter(ch=>ch.id===currentChannelId).map(ch=>{
          const sched=buildSchedule(allPrograms,ch.id);
          const cur=getCurProg(sched);
          return <div key={ch.id} style={{position:"relative",height:ROW_H,borderBottom:"1px solid rgba(255,255,255,0.08)",borderLeft:"3px solid #1a73e8",width:totalW,background:"rgba(26,115,232,0.08)"}}>
            {sched.filter(p=>Number(p.horarioFim)<=86400&&!p.isPlaceholder).map(prog=>{
              ...
              return <div key={prog.id} onClick={()=>{onSelectChannel(ch.id);onSelectProgram(prog)}}
                style={{position:"absolute",left,width:w,top:0,bottom:2,cursor:"pointer",overflow:"hidden",background:isNow?"rgba(60,70,90,0.95)":"rgba(35,40,55,0.7)",borderRight:"1px solid rgba(255,255,255,0.06)",borderLeft:"1px solid rgba(255,255,255,0.03)",boxSizing:"border-box",transition:"background 0.2s"}}
                onMouseEnter={e=>e.currentTarget.style.background=isNow?"rgba(70,85,110,1)":"rgba(50,60,75,0.9)"}
                onMouseLeave={e=>e.currentTarget.style.background=isNow?"rgba(60,70,90,0.95)":"rgba(35,40,55,0.7)"}>
